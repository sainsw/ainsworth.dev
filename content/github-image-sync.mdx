...
flowchart TD
  ...
  C --> D[Node script: optimise with Sharp (resize → WebP)]
  D --> E[Compute content hash]
  E -->|unchanged| F[Keep filename → leverage Cloudflare/Vercel/browser cache]
  E -->|changed| G[Write new hashed filename → instant cache-bust]
  F --> H[Publish]
  G --> H[Publish]
...
## Related Reading
	•	AI Coding Tools: My Journey From Frustration to Flow — how I decide what work to hand to automation.
