...
flowchart TB
  ...
  %% Minor change path
  A["Edit resume.json"] --> G["generate-latex.js builds cv-source/main.tex"]
  ...
  %% Build PDF locally
  G --> P["pdflatex local outputs public slash files slash cv-HASH dot pdf"]
  P --> K["Keep old PDFs in public slash files"]
  P --> C["git add and commit"]
  C --> U["push"]
  ...
  subgraph VercelBuild["Vercel build - no LaTeX"]
    ...
    V3 --> V4["Next build links to slash files slash cv-CV_VERSION dot pdf"]
  end
