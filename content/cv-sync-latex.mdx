...
flowchart TB
  ...
  A["Edit resume.json"]:::file --> G["generate-latex.js builds cv-source/main.tex"]:::local
  D -- "Minor content or layout" --> A

  %% Theme overhaul path (Overleaf-driven)
  O["Draft in Overleaf – new or updated template"]:::local --> O2["Export main.tex back to repo"]:::local --> G
  D -- "Theme overhaul" --> O

  %% Build PDF locally
  G --> P["pdflatex - local outputs public/files/cv-HASH.pdf"]:::local
  P --> K["Keep old PDFs in /public/files"]:::local
  P --> C["git add and commit"]:::local
  C --> U["push"]:::local

  %% Vercel side
  subgraph VercelBuild["Vercel build - \"no LaTeX\""]
    direction TB
    V1["Install dependencies"]:::vercel --> V2["Skip LaTeX build"]:::vercel
    V2 --> V3["generate-version.js computes CV_VERSION from main.tex and class file"]:::vercel
    V3 --> V4["Next build links to /files/cv-CV_VERSION.pdf"]:::vercel
  end

  U --> V1
  V4 --> S["Users download latest PDF - cache-busted by filename"]:::vercel
```
