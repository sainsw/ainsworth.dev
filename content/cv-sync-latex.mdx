...
```mermaid
flowchart TD
  A["resume.json"] --> B["generate-latex.js → cv-source/main.tex"]
  B --> C["pdflatex (local) → public/files/cv-<hash>.pdf"]
  C --> D["git add/commit old + new PDFs"]
  D --> E["push"]
  E --> F["Vercel build → generate-version.js"]
  F --> G["lib/version.js: export const CV_VERSION='<hash>'"]
  G --> H["Front end links /files/cv-${CV_VERSION}.pdf"]
```
