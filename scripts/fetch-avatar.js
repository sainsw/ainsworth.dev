...
async function getExistingAvatarHash() {
  ...
  if (avatarFiles.length > 0) {
    ...
    const preferredFile =
      sortedFiles.find((f) => f.endsWith(".jpg")) || sortedFiles[0];
...
async function convertWithSharp(inputPath) {
  try {
    ...
    const TEMP_WEBP_PATH = path.join(
      OUTPUT_DIR,
      `avatar-temp-${tempHash}.webp`,
    );
    const TEMP_JPG_PATH = path.join(OUTPUT_DIR, `avatar-temp-${tempHash}.jpg`);

    // Create optimised WebP (priority format)
    ...
    // Generate final hash from optimised JPG (most stable format)
    ...
    return {
      success: true,
      hash: finalHash,
      webpPath: FINAL_WEBP_PATH,
      jpgPath: FINAL_JPG_PATH,
    };
  } catch (error) {
    console.log(
      `ℹ️  Sharp not available (${error.message}), using original format`,
    );
...
async function fetchGitHubAvatar() {
  try {
    ...
    // Convert and optimise (prioritising WebP)
    ...
    if (result.success) {
      console.log(
        `📁 WebP: ${path.relative(process.cwd(), result.webpPath)} (optimized)`,
      );
      console.log(
        `📁 JPG: ${path.relative(process.cwd(), result.jpgPath)} (fallback)`,
      );
      ...
      console.log(
        `🔍 WebP size: ${webpStats.size} bytes, modified: ${webpStats.mtime}`,
      );
      console.log(
        `🔍 JPG size: ${jpgStats.size} bytes, modified: ${jpgStats.mtime}`,
      );
    } else {
      console.log(
        `📁 JPG: ${path.relative(process.cwd(), result.jpgPath)} (updated)`,
      );
    ...
    if (existingHash !== "fallback") {
      ...
      const versionedWebp = path.join(
        OUTPUT_DIR,
        `avatar-${existingHash}.webp`,
      );
