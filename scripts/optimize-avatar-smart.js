...
async function optimizeAvatar() {
  ...
  try {
    ...
    // Create optimised WebP version (primary) with smart cropping
    ...
    // Create optimised JPEG fallback with same settings
    await sharp(originalPath)
      .rotate(90) // 90 degrees clockwise
      .resize(160, 160, {
        fit: "cover",
    ...
    console.log(
      `✅ WebP: ${Math.round(webpStats.size / 1024)}KB (smart cropped + rotated)`,
    );
    console.log(
      `✅ JPEG fallback: ${Math.round(jpegStats.size / 1024)}KB (smart cropped + rotated)`,
    );
    console.log(
