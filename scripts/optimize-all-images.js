...
async function optimizeAllImages() {
  ...
  for (const filename of asfc_files) {
    ...
    if (fs.existsSync(inputPath)) {
      ...
      // Create optimized PNG fallback (much smaller)
      await sharp(inputPath)
        .resize(targetSize, targetSize, {
          fit: 'contain',
      ...
      const webpSize = fs.statSync(path.join(logoDir, filename.replace('.png', '.webp'))).size;
      
      console.log(`✅ ${filename}: ${Math.round(originalSize/1024)}KB → ${Math.round(newSize/1024)}KB PNG + ${Math.round(webpSize/1024)}KB WebP`);
      console.log(`   📊 Savings: ${Math.round((1 - newSize/originalSize) * 100)}%`);
    }
  }
